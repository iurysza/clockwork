...
function paths(home) {
  ...
  const label = "com.iurysouza.clockwork";
  return {
    home,
...
function renderPlist(target) {
  const template = fs.readFileSync(path.join(SERVICE_DIR, "templates", "com.iurysouza.clockwork.plist.tpl"), "utf8");
  return template
...
function installIntegration(target) {
  ...
  for (const relative of ["", "logs", "locks", "delivery-receipts"]) {
  ...
  linkManaged(path.join(SERVICE_DIR, "self-email-once.mjs"), path.join(binDir, "clockwork-self-email-once"), releaseRoot);
...
function doctor(target) {
  ...
  check("Clockwork delivery guard syntax", process.execPath, ["--check", path.join(SERVICE_DIR, "self-email-once.mjs")]);
